...
class FiltersByStoreNameControllerTest @Autowired constructor(val mockMvc: MockMvc) {
    ...
    fun `When get filters store name without filters Then return all stores`() {
        mockMvc.perform(
            post("/filters/STORE_NAME?page=0")
            ...
            .andExpect(jsonPath("$.[0]").value("A Sunday store"))
            .andExpect(jsonPath("$.[9]").value("An unpaged store"))
    ...
    fun `When get filters store name by season Then return stores by season filtered`() {
        mockMvc.perform(
            ...
            .andExpect(jsonPath("$.[0]").value("A faucial store"))
            .andExpect(jsonPath("$.[3]").value("An unpaged store"))
    }
    ...
    fun `When get filters store name by cluster Then return store name by cluster filtered`() {
        ...
        mockMvc.perform(
            ...
            .andExpect(jsonPath("$.length()").value(4))
            .andExpect(jsonPath("$.[0]").value("A Sunday store"))
            .andExpect(jsonPath("$.[3]").value("An earnest store"))
    }
    ...
    fun `When get filters season by region Then return season by cluster filtered`() {
        mockMvc.perform(
            ...
            .andExpect(jsonPath("$.[0]").value("A Sunday store"))
            .andExpect(jsonPath("$.[2]").value("A faucial store"))
    ...
    fun `When get filters season by product Then return season by product filtered`() {
        mockMvc.perform(
            ...
            .andExpect(jsonPath("$.[0]").value("An unpaged store"))
    }
