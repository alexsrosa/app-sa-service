...
class FiltersByProductSkuControllerTest @Autowired constructor(val mockMvc: MockMvc) {

    val mapper = jacksonObjectMapper()

    @Test
    fun `When get filters product sku without filters Then return all products models`() {
        mockMvc.perform(
            post("/filters/SKU?page=0")
            ...
            .andExpect(jsonPath("$.[0]").value("12000001-XS"))
            .andExpect(jsonPath("$.[3]").value("12000005-M"))
    ...
    @Test
    fun `When get filters product sku by season Then return products by season filtered`() {
        mockMvc.perform(
            post("/filters/SKU?page=0")
    ...
    fun `When get filters product model by model Then return season by model filtered`() {
        mockMvc.perform(
            ...
            .andExpect(jsonPath("$.[0]").value("12000001-XS"))
            .andExpect(jsonPath("$.[1]").value("12000001-XXS"))
    }
    ...
    fun `When get filters product by size Then return product by size filtered`() {
        mockMvc.perform(
            ...
            .andExpect(content().contentType(MediaType.APPLICATION_JSON))
            .andExpect(jsonPath("$.length()").value(2))
            .andExpect(jsonPath("$.[0]").value("12000001-XXS"))
            .andExpect(jsonPath("$.[1]").value("12000002-XXS"))
    }
